...
def trainModel(train_x, train_y, model_name='NB', validation=None):
    ...
    if validation is not None:
        ...
        print("Y PREDICTED", y_hat)
        acc = metrics.accuracy_score(validation[1], y_hat)
        print(f"Validation Accuracy in '{model_name}' = {acc}")
        cm = metrics.confusion_matrix(validation[1], y_hat)
        print(cm)
        # Compute precision
        precision = precision_score(validation[1], y_hat, average='weighted', zero_division=1)

        # Compute recall
        recall = recall_score(validation[1], y_hat,average='weighted', zero_division=1)

        # Compute F1 score
        f1 = f1_score(validation[1], y_hat,average='weighted', zero_division=1)
...
testLabels = np.array(['WALKING', 'WALKING', 'WALKING', 'SITTING', 'SITTING', 'SITTING', 'WALKING',
 'WALKING', 'WALKING', 'SITTING', 'SITTING', 'WALKING', 'WALKING', 'WALKING',
 'WALKING', 'WALKING', 'SITTING', 'SITTING', 'SITTING', 'SITTING', 'SITTING'
 'SITTING', 'WALKING', 'WALKING', 'WALKING', 'WALKING', 'WALKING', 'WALKING',
 'LAYING', 'WALKING_DOWNSTAIRS', 'WALKING', 'WALKING', 'WALKING', 'WALKING',
 'WALKING', 'WALKING', 'WALKING', 'LAYING', 'WALKING_DOWNSTAIRS',
 'WALKING_DOWNSTAIRS', 'WALKING_DOWNSTAIRS', 'WALKING_DOWNSTAIRS',
 'WALKING_DOWNSTAIRS', 'WALKING_DOWNSTAIRS', 'WALKING_DOWNSTAIRS', 'WALKING',
 'WALKING', 'WALKING', 'WALKING', 'WALKING', 'WALKING'])
y_test = pd.DataFrame(testLabels, columns=["ActivityName"])
print((y_test))
# dfTest.ActivityName
...
model3 = trainModel(X_train, y_train, model_name='LSVC', validation=(X_test, y_test))
