...
endTimestamp = "2023-05-12 17:02:28.000000"
...
listAccData = list(accelerometerDataCollection.find({ "userId": os.getenv('userId'), "timestamp":{"$gte": dtMongoStart, "$lte": dtMongoEnd} }))
listGyroData = list(gyroscopeDataCollection.find({ "userId": os.getenv('userId'), "timestamp":{"$gte": dtMongoStart, "$lte": dtMongoEnd} }))#, "timestamp":{"$gte": dtFiveSec, "$lte": dtMongo}
...
print(testDataDF)
testDataDF.to_csv('/mnt/c/Users/sltnm/Desktop/FACULTATE/LICENTA/data_processing_calaid/phase2/merged2.csv', encoding='utf-8', index=False)
