...
user = userCollection.find_one({"mongoUserId": os.getenv('userId')})
userWeight = float(user["weight"])
userHeight = user["height"]

timestamps = []
for document in accelerometerDataCollection.find({"userId": os.getenv('userId')}):
    # for key in document:
...
speed = calculateSpeed(accelerometerDataCollection.find({"userId": os.getenv('userId')}), max(timestamps).timestamp())
